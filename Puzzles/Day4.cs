...
namespace advent_of_code_2020
{
    ...
    class Passport
    {
        ...
        public Passport(string inputString, string[] validFields)
        {
        ...
        public bool isValid()
        {
            ...
            {
                Console.WriteLine("byr:{0} ({1}) iyr:{2} ({3}) eyr:{4} ({5}) hgt:{6} ({7}) hcl:{8} ({9}) ecl:{10} ({11}) pid:{12} ({13}), cid:{14} ({15})",
                GetPassportProperty("byr"), ByrValid(),
                GetPassportProperty("iyr"), IyrValid(),
                GetPassportProperty("eyr"), EyrValid(),
                GetPassportProperty("hgt"), HgtValid(),
                GetPassportProperty("hcl"), HclValid(),
                GetPassportProperty("ecl"), EclValid(),
                GetPassportProperty("pid"), PidValid(),
                GetPassportProperty("cid"), CidValid()
                 );
            ...
            return foundFields.Count() == ValidFields.Count();
        }
        public string GetPassportProperty(string passportProperty)
        {
            var props = InputString.Split(" ").ToList();
            foreach (string propString in props)
            {
                if (propString.Contains(passportProperty))
                {
                    var propValue = propString.Split(":")[1];
                    return propValue;
                }
            }
        ...
        {
            var byr = GetPassportProperty("byr");

            if (byr.Length == 4)
            {
                var byrInt = Convert.ToInt32(byr);
        ...
        {
            var iyr = GetPassportProperty("iyr");
            var rx = new Regex(@"[0-9]{4}$");
            if (rx.IsMatch(iyr))
            {
                var iyrInt = Convert.ToInt32(iyr);
        ...
        {
            var eyr = GetPassportProperty("eyr");
            if (eyr.All(char.IsDigit) && eyr.Length == 4)
            {
                var eyrInt = Convert.ToInt32(eyr);
        ...
        {
            var hgt = GetPassportProperty("hgt");
            
            if ((new Regex(@"(\d*)(cm|in)")).IsMatch(hgt))
            {
                var heightValue = Convert.ToInt32((new Regex(@"(\d*)")).Match(hgt).ToString());
                var unit = (new Regex(@"(cm|in)")).Match(hgt).ToString();
                if (unit == "cm")
                {
        ...
        {
            var rx = new Regex(@"^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$");
            var hexValue = GetPassportProperty("hcl");
            return rx.IsMatch(hexValue);
        }
        public bool EclValid()
        {
            return (new Regex(@"^[amb|blu|brn|gry|grn|hzl|oth]{3}$")).IsMatch(GetPassportProperty("ecl"));
        }

        public bool PidValid()
        {
            return (new Regex(@"([0-9]){9}$")).IsMatch(GetPassportProperty("pid"));
        }
